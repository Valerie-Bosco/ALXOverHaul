...
class ALX_OT_Mesh_Cleanup(bpy.types.Operator):
    bl_label = ""
    bl_idname = "alx.operator_mesh_cleanup"
    ...
    distance: FloatProperty(
        name="Merge Distance", default=0.0001, min=0, step=0.01, precision=4
    )
    ...
    dissolve_redundant_verts: BoolProperty(
        name="Dissolve Redundant Verts", default=True
    )
    dissolve_redundant_edges: BoolProperty(
        name="Dissolve Redundant Edges", default=False
    )
    dissolve_redundant_angle: FloatProperty(
        name="Dissolve Redundnat Angle",
        default=179.999,
        min=0,
        max=180,
        step=0.1,
    ...
    select_type: EnumProperty(
        name="Select", items=cleanup_select_items, default="NON-MANIFOLD"
    )
    planar_threshold: FloatProperty(
        name="Non-Planar Face Threshold", default=0.001, min=0, step=0.0001, precision=6
    )
    ...
    def invoke(self, context, event):
        self.coords = Vector((event.mouse_region_x, event.mouse_region_y)) + Vector(
            (10, 20)
        )
    ...
    def execute(self, context):
        ...
        for obj in sel:
            ...
            if elementcounts != cleanedcounts:
                removed[obj] = (
                    elementcounts[0] - cleanedcounts[0],
        ...
        if removed:
            ...
            if is_any_non_manifold:
                self.report(
                    {"INFO"}, f"Non-Manifold Edges found!\n{text}\n{self.coords}"
                )
    ...
    def delete_loose_geometry(self, bm):
        ...
        if self.delete_loose_faces:
            loose_faces = [
                f for f in bm.faces if all([not e.is_manifold for e in f.edges])
            ]
            if loose_faces:
    ...
    def select_geometry(self, bm):
        ...
        if self.select_type == "NON-MANIFOLD":
            ...
            for f in faces:
                distances = [
                    distance_point_to_plane(v.co, f.calc_center_median(), f.normal)
                    for v in f.verts
                ]
