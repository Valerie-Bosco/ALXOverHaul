    ...
    def f3(self, active, bm):
                ...
                if self.automerge:
                    nonmanifoldverts = [
                        ...
                        if any([not e.is_manifold for e in v.link_edges])
                           and v not in [vs, v_new, v1_other, v2_other]
                    ]

                    if nonmanifoldverts:
                        distance = min(
                            [((v_new.co - v.co).length, v) for v in nonmanifoldverts],
                            key=lambda x: x[0],
                        )
                        threshold = min(
                            [
                                (v_new.co - v.co).length * 0.5
                                for v in [v1_other, v2_other]
                            ]
                        )
                        ...
                        if distance[0] < threshold:
                            ...
                            bmesh.ops.pointmerge(
                                bm, verts=[v_new, v_closest], merge_co=v_closest.co
                            )

                if any([len(v1_other.link_edges) == 4, len(v2_other.link_edges) == 4]):
                    if len(v1_other.link_edges) == 4 and any(
                            [not e.is_manifold for e in v1_other.link_edges]
                    ):
                        vs.select = False
                        v1_other.select = True
                        vs = v1_other

                    elif len(v2_other.link_edges) == 4 and any(
                            [not e.is_manifold for e in v2_other.link_edges]
                    ):
                        vs.select = False
                    ...
                    second_vs = [
                        e.other_vert(vs)
                        for e in vs.link_edges
                        if not e.is_manifold
                           and len(e.other_vert(vs).link_edges) == 4
                           and sum(
                            [not e.is_manifold for e in e.other_vert(vs).link_edges]
                        )
                           == 2
                    ]
        ...
        if len(verts) == 2:
            ...
            if v1_edges and v2_edges:
                ...
                if len(v2_other.link_edges) == 4 and any(
                        [not e.is_manifold for e in v2_other.link_edges]
                ):
                    v2_other.select = True
                ...
                if v1_other.select and not v2_other.select:
                    ...
                    second_vs = [
                        ...
                        if not e.is_manifold
                           and e.other_vert(v1) != v2
                           and len(e.other_vert(v1).link_edges) == 4
                    ]
                    if second_vs:
                        second_v = second_vs[0]
                        second_v.select = True
                    ...
                    second_vs = [
                        ...
                        if not e.is_manifold
                           and e.other_vert(v2) != v1
                           and len(e.other_vert(v2).link_edges) == 4
                    ]
                    if second_vs:
                        second_v = second_vs[0]
                        second_v.select = True
