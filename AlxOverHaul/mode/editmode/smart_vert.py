...
def draw_slide_status(op):
    def draw(self, context):
        ...
        text = (
            f"Slide Extend Snap to {op.snap_element.capitalize()}"
            if op.snap_element
            else "Slide Extend"
        )
        row.label(text=text)
...
class ALX_OT_Mesh_SmartVertex(bpy.types.Operator):
    bl_label = "ALX Mesh - Smart Vertex"
    bl_idname = "alx.operator_mesh_smart_vertex"
    bl_options = {"REGISTER", "UNDO"}

    mode: EnumProperty(name="Mode", items=smartvert_mode_items, default="MERGE")
    merge_type: EnumProperty(
        name="Merge Type", items=smartvert_merge_type_items, default="LAST"
    )
    merge_center_paths: BoolProperty(name="Merge Paths in center", default=False)
    pathtype: EnumProperty(
        name="Path Type", items=smartvert_path_type_items, default="TOPO"
    )

    slide_override: BoolProperty(name="Slide Override", default=False)
    vert_bevel: BoolProperty(name="Single Vertex Bevelling", default=False)

    index: IntProperty(
        name="Index of Edge associated with HyperCursor Gizmo, that is to be removed"
    )
    ...
    def invoke(self, context, event):
        ...
        if self.slide_override:
            ...
            if context.mode == "EDIT_MESH":
                ...
                if tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (
                        True,
                        False,
                        False,
                ):
                    ...
                    if len(selected) == 1:
                        ...
                        if (
                            ...
                            last = history[-1]
                            self.verts = {
                                v: {"co": v.co.copy(), "target": last}
                                for v in selected
                                if v != last
                            }

                elif tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (
                        False,
                        True,
                        False,
                ):
                    ...
                    for edge in selected:
                        edge_center = average_locations(
                            [self.mx @ v.co for v in edge.verts]
                        )
                        ...
                        self.verts[closest] = {
                            "co": closest.co.copy(),
                            "target": edge.other_vert(closest),
                        }
            ...
            if len(self.verts) == 1:
                ...
                planar_edges = [
                    ...
                    if not e.other_vert(vert) == self.verts[vert]["target"]
                ]

                if len(planar_edges) == 2:
                    end_faces = [
                        f
                        for f in planar_edges[0].link_faces
                        if f in planar_edges[1].link_faces
                    ]
                    ...
                    if len(end_faces) == 1:
                        ...
                        if len(end_face.verts) > 3:
                            ...
                            other_verts = [
                                v for v in end_face.verts if v not in tri_verts
                            ]

                            for v in other_verts:
                                slide_edges = [
                                    e for e in v.link_edges if e not in end_face.edges
                                ]

                                if slide_edges:
                                    line = [
                                        slide_edges[0].verts[0].co.copy(),
                                        slide_edges[0].verts[1].co.copy(),
                                    ]
                                    self.flatten_dict["other_verts"][v] = {
                                        "co": v.co.copy(),
                                        "line": line,
                                    }
    ...
    def smart_vert(self, context):
        ...
        if len(verts) == 1 and tuple(
            ...
            elif self.merge_type == "PATHS" and tuple(
                    bpy.context.scene.tool_settings.mesh_select_mode
            ) == (True, False, False):
                if len(verts) == 4:
        ...
        elif self.mode == "CONNECT" and tuple(
                bpy.context.scene.tool_settings.mesh_select_mode
        ) == (True, False, False):
            if len(verts) == 4:
                history = self.validate_history(active, bm)
        ...
        if faces:
            ...
            for verts, _, _ in islands:
                ...
                bmesh.ops.pointmerge(
                    bm,
                    verts=merge_verts,
                    merge_co=average_locations([v.co for v in merge_verts]),
                )
            ...
            for verts, _ in sequences:
                bmesh.ops.pointmerge(
                    bm, verts=verts, merge_co=average_locations([v.co for v in verts])
                )
    ...
    def slide(self, context):
        ...
        self.distance = (
                                self.mx.to_3x3().inverted_safe() @ (self.init_loc - self.loc)
                        ).length * origin_dir.dot(move_dir)
    ...
    def slide_snap(self, context):
        ...
        face_distance = (
            hitface,
            (hit_co - hitface.calc_center_median_weighted()).length / face_weight,
        )
        ...
        edge = min(
            [
                (
                    ...
                    (
                            hit_co
                            - intersect_point_line(hit_co, e.verts[0].co, e.verts[1].co)[0]
                    ).length,
        ...
        edge_distance = (
            edge[0],
            ((edge[1] * edge[2]) / edge[0].calc_length()) / edge_weight,
        )
