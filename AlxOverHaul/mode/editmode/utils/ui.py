...
def wrap_cursor(self, context, event, x=False, y=False):
    if x:
        ...
        if event.mouse_region_x <= 0:
            context.window.cursor_warp(
                context.region.width + self.region_offset_x - 10, event.mouse_y
            )
    ...
    if y:
        if event.mouse_region_y <= 0:
            context.window.cursor_warp(
                event.mouse_x, context.region.height + self.region_offset_y - 10
            )
...
def get_zoom_factor(context, depth_location, scale=10, ignore_obj_scale=False):
    ...
    try:
        center_3d = region_2d_to_location_3d(
            context.region, context.region_data, center, depth_location
        )
        offset_3d = region_2d_to_location_3d(
            context.region, context.region_data, offset, depth_location
        )
    except:
        print("exception!")
        return 1
    ...
    if not ignore_obj_scale and context.active_object:
        ...
        zoom_vector = mx.inverted_safe() @ Vector(
            ((center_3d - offset_3d).length, 0, 0)
        )
...
def get_flick_direction(context, mouse_loc_3d, flick_vector, axes):
    ...
    for direction, axis in axes.items():
        ...
        axis_2d = location_3d_to_region_2d(
            context.region, context.region_data, mouse_loc_3d + axis, default=origin_2d
        )
    ...
    return min(
        [(d, abs(flick_vector.xy.angle_signed(a))) for d, a in axes_2d.items()],
        key=lambda x: x[1],
    )[0]
...
def require_header_offset(context, top=True):
    ...
    headers = [
        ...
        if r.type == ("HEADER" if bpy.app.version < (3, 0, 0) else "TOOL_HEADER")
           and ((r.y > area.height / 2) if top else (r.y < area.height / 2))
    ]
...
def get_keymap_item(
        name, idname, key=None, alt=False, ctrl=False, shift=False, properties=[]
):
            ...
            found = (
                ...
                if key is None
                    ...
                    [
                        kmi.type == key
                        and kmi.alt is alt
                        and kmi.ctrl is ctrl
                        and kmi.shift is shift
                    ]
                )
            )
...
def init_timer_modal(self, debug=False):
    ...
    if debug:
        print(
            f"initiating timer with a countdown of {self.time}s ({self.time * get_prefs().modal_hud_timeout}s)"
        )
