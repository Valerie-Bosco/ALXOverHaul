...
def cast_bvh_ray_from_mouse(
        mousepos, candidates=None, bmeshes={}, bvhs={}, debug=False
):
...
def cast_obj_ray_from_mouse(mousepos, depsgraph=None, candidates=None, debug=False):
    ...
    for obj in objects:
        ...
        success, location, normal, index = obj.ray_cast(
            origin=ray_origin, direction=ray_direction, depsgraph=depsgraph
        )
        distance = (mx @ location - origin_3d).length
...
def get_closest(origin, candidates=[], depsgraph=None, debug=False):
    ...
    for obj in objects:
        ...
        if obj_eval.data.polygons:
            success, location, normal, index = obj.closest_point_on_mesh(
                origin_local, depsgraph=depsgraph
            )
            ...
            if distance is not None and distance < nearestdistance:
                (
                    ...
                    nearestindex,
                    nearestdistance,
                ) = (obj, mx @ location, mx.to_3x3() @ normal, index, distance)
    ...
    if nearestobj:
        return (
            nearestobj,
            nearestobj.evaluated_get(depsgraph),
            nearestlocation,
            nearestnormal,
            nearestindex,
            nearestdistance,
...
def cast_scene_ray_from_mouse(
        mousepos, depsgraph, exclude=[], exclude_wire=False, unhide=[], debug=False
):
    region = bpy.context.region
    region_data = bpy.context.region_data
    ...
    hit, location, normal, index, obj, mx = scene.ray_cast(
        depsgraph=depsgraph, origin=view_origin, direction=view_dir
    )
    ...
    if hit:
        if obj in exclude or (exclude_wire and obj.display_type == "WIRE"):
            ...
            while ignore:
                ...
                if hit:
                    ignore = obj in exclude or (
                            exclude_wire and obj.display_type == "WIRE"
                    )
                else:
                    break
