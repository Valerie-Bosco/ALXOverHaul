...
def create_rotation_matrix_from_vertex(obj, vert):
    ...
    if vert.link_edges:
        ...
        binormal = (
                mx.to_3x3() @ (longest_edge.other_vert(vert).co - vert.co)
        ).normalized()
...
def create_rotation_matrix_from_edge(context, mx, edge):
    ...
    if edge.link_faces:
        normal = average_normals(
            [get_world_space_normal(f.normal, mx) for f in edge.link_faces]
        ).normalized()
...
def create_rotation_matrix_from_face(
        context,
        mx,
        face,
        edge_pair=True,
    ...
    if len(face.verts) > 4:
        ...
        edges_are_same_length = all(
            [
                abs(l - avg_edge_length) < avg_edge_length * cylinder_threshold
                for l in edge_lengths
            ]
        )
        verts_have_same_center_distance = all(
            [
                abs(d - avg_center_distance) < avg_center_distance * cylinder_threshold
    ...
    if circle:
        for axis in [Vector((0, 1, 0)), Vector((1, 0, 0)), Vector((0, 0, 1))]:
            ...
            i = intersect_line_plane(
                face_center + axis,
                face_center + axis + face.normal,
                face_center,
                face.normal,
            )
    ...
    if not binormal:
        binormal = (
            (mx.to_3x3() @ face.calc_tangent_edge_pair()).normalized()
            if edge_pair
            else (mx.to_3x3() @ face.calc_tangent_edge()).normalized()
        )
