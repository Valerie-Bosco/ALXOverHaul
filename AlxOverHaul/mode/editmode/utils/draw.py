    ...
    def draw():
        ...
        if indices:
            if mx != Matrix():
                ...
                batch = batch_for_shader(
                    shader, "POINTS", {"pos": coords}, indices=indices
                )

        else:
            if mx != Matrix():
                batch = batch_for_shader(
                    shader, "POINTS", {"pos": [mx @ co for co in coords]}
                )
            else:
...
def draw_line(
    ...
    def draw():
        ...
        batch = batch_for_shader(
            shader, "LINES", {"pos": [mx @ co for co in coords]}, indices=indices
        )
        batch.draw(shader)
...
def draw_circle(
    ...
    def draw():
        ...
        batch = batch_for_shader(
            shader, "LINES", {"pos": [rot @ co for co in coords]}, indices=indices
        )
        batch.draw(shader)
...
def draw_cross_3d(
        co, mx=Matrix(), color=(1, 1, 1), width=1, length=1, alpha=1, xray=True, modal=True
):
    def draw():
...
def draw_tris(
        coords, indices=None, mx=Matrix(), color=(1, 1, 1), alpha=1, xray=True, modal=True
):
    def draw():
        ...
        if mx != Matrix():
            batch = batch_for_shader(
                shader, "TRIS", {"pos": [mx @ co for co in coords]}, indices=indices
            )
...
def draw_split_row(
    ...
    text = (
        text
        if text
        else str(getattr(self, prop)) if getattr(self, prop) in [True, False] else ""
    )
...
def draw_axes_HUD(context, objects):
    ...
    if context.space_data.overlay.show_overlays:
        ...
        show_hyper_cursor = (
                hypercursor
                and get_active_tool(context).idname
                in ["machin3.tool_hyper_cursor", "machin3.tool_hyper_cursor_simple"]
                and context.scene.HC.show_gizmos
        )
            ...
            for obj in objects:
                ...
                if obj == "CURSOR":
                    ...
                    if not show_hyper_cursor:
                        ...
                        factor = (
                            get_zoom_factor(
                                context, origin, scale=300, ignore_obj_scale=True
                            )
                            if screenspace
                            else 1
                        )
                        ...
                        if show_cursor and screenspace:
                            coords.append(
                                origin
                                + (rot @ axis).normalized() * 0.1 * scale * factor * 0.8
                            )
                            coords.append(
                                origin
                                + (rot @ axis).normalized() * 0.1 * scale * factor * 1.2
                            )
                            ...
                            coords.append(
                                origin
                                + (rot @ axis).normalized() * size * scale * factor
                            )
                    ...
                    coords.append(
                        origin + (rot @ axis).normalized() * size * scale * factor * 0.1
                    )
                    coords.append(
                        origin + (rot @ axis).normalized() * size * scale * factor
                    )
    ...
    if context.space_data.overlay.show_overlays:
        ...
        if view.local_view:
            ...
            shader = gpu.shader.from_builtin(
                get_builtin_shader_name("UNIFORM_COLOR", "2D")
            )
            shader.bind()
            ...
            blf.color(font, *color, alpha)
            blf.position(
                font, center - (dims[0] / 2), region.height - offset - fontsize, 0
            )
...
def draw_surface_slide_HUD(context, color=(1, 1, 1), alpha=1, width=2):
    if context.space_data.overlay.show_overlays:
        ...
        blf.color(font, *color, alpha)
        blf.position(
            font, (region.width / 2) - int(60 * scale), 0 + offset + int(fontsize), 0
        )
...
def draw_screen_cast_HUD(context):
    ...
    bottom_header = [
        ...
        if r.type == "HEADER" and r.alignment == "BOTTOM"
    ]
    bottom_tool_header = [
        r
        for r in context.area.regions
        if r.type == "TOOL_HEADER" and r.alignment == "BOTTOM"
    ]
    ...
    for idx, (addon, label, idname, prop) in enumerate(reversed(operators)):
        ...
        color = (
            green
            if idname.startswith("machin3.") and p.screencast_highlight_machin3
            else white
        )
