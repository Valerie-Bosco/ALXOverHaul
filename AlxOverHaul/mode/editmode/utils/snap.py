...
class Snap:
    ...
    def get_hit(self, mousepos):
        ...
        (
            ...
            self.hitnormal,
            self.hitmx,
        ) = cast_scene_ray_from_mouse(
            mousepos,
            self.depsgraph,
            exclude=self.exclude,
            exclude_wire=self.exclude_wire,
            unhide=self.alternative,
            debug=self.debug,
        )
        ...
        if self.hit:
            ...
            if name not in self.cache.objects:
                ...
                mesh = bpy.data.meshes.new_from_object(
                    self.hitobj.evaluated_get(self.depsgraph), depsgraph=self.depsgraph
                )
            ...
            if not self.hitface or (
                    self.hitface and self.hitface.index != self.hitindex
            ):
            ...
            if self.hitindex not in self.cache.tri_coords[name]:
                ...
                tri_coords = [
                    self.hitmx @ l.vert.co
                    for tri in loop_triangles
                    if tri[0].face == self.hitface
                    for l in tri
                ]
    ...
    def _init_exclude(self, context, include, exclude, exclude_wire):
        if include:
            self.exclude = [
                obj for obj in context.visible_objects if obj not in include
            ]
        ...
        if view.local_view:
            hidden = [
                obj for obj in context.view_layer.objects if not obj.visible_get()
            ]
            self.exclude += hidden
    ...
    def _update_meshes(self, context):
        ...
        self._edit_mesh_objs = [
            obj for obj in context.visible_objects if obj.mode == "EDIT"
        ]
...
class SnapCache:
    ...
    def clear(self):
        for name, mesh in self.meshes.items():
            self.log(
                f" Removing {name}'s temporary snapping mesh {mesh.name} with {len(mesh.polygons)} faces and {len(mesh.vertices)} verts"
            )
