def get_selected_vert_sequences(verts, ensure_seq_len=False, debug=False):
    ...
    noncyclicstartverts = [
        v for v in verts if len([e for e in v.link_edges if e.select]) == 1
    ]
...
def get_edges_vert_sequences(verts, edges, debug=False):
    ...
    noncyclicstartverts = [
        v for v in verts if len([e for e in v.link_edges if e in edges]) == 1
    ]
    ...
    while verts:
        ...
        if nextv:
            ...
            cyclic = (
                True if len([e for e in v.link_edges if e in edges]) == 2 else False
            )
...
def get_boundary_edges(faces, region_to_loop=False):
    boundary_edges = [
        ...
        for f in faces
        for e in f.edges
        if (not e.is_manifold) or (any(not f.select for f in e.link_faces))
    ]
