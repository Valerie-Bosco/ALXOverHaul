...
class Alx_OT_Scene_VisibilityIsolator(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        AddonProperties: Alx_PG_PropertyGroup_SessionProperties = (
            context.window_manager.alx_session_properties
        )
        SelectedVisibility = (
            AddonProperties.operator_object_and_collection_isolator_visibility_target
        )
        SelectedType = (
            AddonProperties.operator_object_and_collection_isolator_type_target
        )
            ...
            try:
                ...
                try:
                    ...
                    IsolatorObjects = [
                        ...
                        if (
                                (Object is not None)
                                and (Object not in context.selected_objects)
                        )
                    ]
                    ObjectsCollections = [
                        Object.users_collection[0]
                        for Object in context.selected_objects
                    ]
                    IsolatorCollection = [
                        Collection
                        for ObjectCollection in ObjectsCollections
                        for Collection in bpy.data.collections
                        if (ObjectCollection != Collection)
                           and (ObjectCollection not in Collection.children_recursive)
                    ]
                    ...
                    try:
                        ...
                        try:
                            ...
                            for (
                                    ...
                                    context.scene.alx_scene_isolator_visibility_object_list
                            ):
                                if Object is not None:
                                    if "VIEWPORT" in VisibilityType:
                                        Object.hide_viewport = not self.TargetVisibility
                ...
                if "COLLECTION" in TargetType:
                    try:
                        ...
                        try:
                            context.scene.alx_scene_isolator_visibility_collection_list[
                                0
                            ]
...
class Alx_OT_Object_VisibilitySwitch(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        if self.object_pointer_reference != "":
            Object: bpy.types.Object = bpy.data.objects.get(
                self.object_pointer_reference
            )
            if Object is not None:
