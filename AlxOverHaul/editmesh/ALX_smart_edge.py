...
class ALX_OT_Mesh_SmartEdge(bpy.types.Operator):
    ...
    sharp_mode: EnumProperty(
        name="Sharp Mode", items=smartedge_sharp_mode_items, default="SHARPEN"
    )
    bevel_weight: FloatProperty(name="Weight", default=1, min=0.01, max=1)
    bevel_amount: FloatProperty(name="Amount", default=0.1, min=0, step=1, precision=4)
    ...
    bridge_interpolation: EnumProperty(
        name="Interpolation", items=bridge_interpolation_items, default="SURFACE"
    )
    ...
    select_mode: EnumProperty(
        name="Select Mode", items=smartedge_select_mode_items, default="BOUNDS"
    )
...
#
#         if (
#             self.is_knife_projectable
#             and self.is_knife_project
#             and not self.offset
#             and not self.sharp
#         ):
...
#                             number_cuts=self.bridge_cuts,
#                             interpolation=self.bridge_interpolation,
#                         )
#                         self.draw_bridge_props = True
#                     except:
#                         popup_message(
#                             "SmartEdge in Bridge mode requires two separate, non-manifold edge loops."
#                         )
#                         return {"CANCELLED"}
#
#                 elif 1 <= len(edges) < 4:
...
#
#         connected_edge_counts = [
#             len([e for e in v.link_edges if e not in edges]) for v in verts
#         ]
#
#         for e in edges:
#             e.smooth = True
...
#
#         adjacent = {
#             e for f in faces for v in f.verts for e in v.link_edges if e not in edges
#         }
#
