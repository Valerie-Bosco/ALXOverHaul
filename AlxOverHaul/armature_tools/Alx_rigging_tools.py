...
from ..utilities.AlxUtilities import get_modifiers_of_type, operator_log_warning
...
class Alx_OT_Armature_BoneChainOnSelection(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        if (context.mode == "EDIT_MESH") and (context.edit_object.type == "MESH"):
            ...
            edge_selection = [
                edge.index
                for edge in self.hair_strip_bmesh.edges
                if (edge.select == True)
            ]
            ...
            for edge_index in edge_selection:
                ...
                for vert in self.hair_strip_bmesh.edges[edge_index].verts:
                    for lnk_edge in vert.link_edges:
                        if (lnk_edge.select == True) and (
                                lnk_edge.index
                                != self.hair_strip_bmesh.edges[edge_index].index
                        ):
                            count_check += 1
            ...
            edge_selection_validity = (len(edge_selection_check) > 0) and all(
                edge_selection_check
            )
            ...
            if edge_selection_validity == False:
                self.report(
                    {"ERROR"},
                    "Selection is invalid - no more than two connected edge selected per vertex",
                )
                return {"CANCELLED"}
            ...
            if edge_selection_validity == True:
                chain_ends = [
                    ...
                    for vert in self.hair_strip_bmesh.edges[edge_index].verts
                    if (
                            len([0 for edge in vert.link_edges if (edge.select == True)])
                            == 1
                    )
                ]
                ...
                if (
                        self.hair_strip_bmesh.verts[chain_ends[0]].co.z
                        > self.hair_strip_bmesh.verts[chain_ends[1]].co.z
                    ...
                    for edge in queue_vert.link_edges:
                        if edge.index in edge_selection:
                            for vert in edge.verts:
                                if (vert.index not in ordered_vert_chain) and (
                                        vert.index != queue_vert.index
                                ):
                ...
                for i in range(0, len(ordered_vert_chain) - 1):
                    head = (
                            self.hair_strip_object.matrix_world
                            @ self.hair_strip_bmesh.verts[ordered_vert_chain[i]].co
                    )
                    tail = (
                            self.hair_strip_object.matrix_world
                            @ self.hair_strip_bmesh.verts[ordered_vert_chain[i + 1]].co
                    )
                    vertex_normal = self.hair_strip_bmesh.verts[
                        ordered_vert_chain[i]
                    ].normal
                ...
                while i < len(ordered_vert_chain) - 1:
                    if (
                            self.hair_strip_object.vertex_groups.get(
                                f"bone.v_index_{ordered_vert_chain[i]}"
                            )
                            is None
                    ):
                ...
                if self.user_armature_name != "":
                    ...
                    for i in range(0, len(ordered_vert_chain)):
                        if i < len(ordered_vert_chain) - 1:
                            bone_vertex_group: bpy.types.VertexGroup = (
                                self.hair_strip_object.vertex_groups.get(
                                    f"bone.v_index_{ordered_vert_chain[i]}"
                                )
                            )
                            ...
                            if bone_vertex_group is not None:
                                bone_edge = [
                                    v1_edge
                                    for v1_edge in self.hair_strip_bmesh.verts[
                                        ordered_vert_chain[i]
                                    ].link_edges
                                    if (
                                            v1_edge.index
                                            in [
                                                v2_edge.index
                                                for v2_edge in self.hair_strip_bmesh.verts[
                                            ordered_vert_chain[i + 1]
                                        ].link_edges
                                            ]
                                    )
                                ][0]
                                bone_vertex_group.add(
                                    index=[vert.index for vert in bone_edge.verts],
                                    weight=1,
                                    type="REPLACE",
                                )
                                ...
                                if bone_edge is not None:
                                    ...
                                    while i_rec < 100:
                                        ...
                                        vertex_blacklist = [
                                            *[vert.index for vert in queue_edge.verts],
                                            *[vert.index for vert in queue_edge.verts],
                                        ]
                                        last_edge = []
                                        if len(last_edge) >= 2:
                                                ...
                                                for f_vert in f_edge.verts:
                                                    ...
                                                    if f_edge not in processed_edges:
                                                        bone_vertex_group.add(
                                                            index=[
                                                                vert.index
                                                                for vert in f_edge.verts
                                                            ],
                                                            weight=1,
                                                            type="REPLACE",
                                                        )
                                                        last_edge.append(0)
                                                        queue_edges.append(f_edge)

                                                processed_edges.append(f_edge.index)
                    ...
                    while i < len(ordered_vert_chain):
                        bone = (
                            armature_data.edit_bones.new(
                                f"bone.v_index_{ordered_vert_chain[i]}"
                            )
                            if armature_data.edit_bones.get(
                                f"bone.v_index_{ordered_vert_chain[i]}"
                            )
                               is None
                            else armature_data.edit_bones.get(
                                f"bone.v_index_{ordered_vert_chain[i]}"
                            )
                        )
                        ...
                        if bone is not None:
                            if (
                                    f"bone.v_index_{ordered_vert_chain[i]}"
                                    in ordered_bone_position_map.keys()
                            ):
                                ...
                                bone.align_roll(
                                    vector=ordered_bone_position_map[
                                        f"bone.v_index_{ordered_vert_chain[i]}"
                                    ][2]
                                )
...
class ALX_OT_SKELETON_AutoResolveEndBones(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        if (
                ...
                and (context.active_object.data is not None)
            ...
            for bone in armature_object.edit_bones:
                ...
                if length > 1:
                    if bone.parent is not None:
                        parent_pattern = re.findall(
                            r"(\_end)|(\.end)", bone.parent.name, re.IGNORECASE
                        )
                    ...
                    bone.use_connect = (
                        True if bone.use_connect == False else bone.use_connect
                    )
                    reversed_name = bone.name.lower()[::-1]
                    index = reversed_name.find("dne.")

                    if index != -1:
                        bone.name = f"{reversed_name[:index]}{reversed_name[index:index + 3].lower()}_{reversed_name[index + 4:]}"[
                            ::-1
                        ]
