    ...
    def execute(self, context: bpy.types.Context):
        if (context.active_object is not None) and (
            ...
            [
                ...
                if ((shapekey.lock_shape == False) and (shapekey.name != "Basis"))
            ]
    ...
    def execute(self, context: bpy.types.Context):
        if (context.active_object is not None) and (
            ...
            [
                ...
                for shapekey in keyblocks
                if (
                    (shapekey.lock_shape == False)
                    and (keyblocks.keys().count(shapekey.name))
            )
            ]
...
class Alx_OT_Shapekey_TransferShapekeysToTarget(bpy.types.Operator):
    ...
    def auto_retrieve_shapekeys(self, context: bpy.types.Context):
        ...
        shape_key_object: bpy.types.Object = (
            context.window_manager.alx_session_properties.shapekey_transfer_source_object
        )

        if (
                (shape_key_object is not None)
                and (shape_key_object.data.shape_keys is not None)
                and (len(shape_key_object.data.shape_keys.key_blocks) != 0)
        ):
            key_blocks = shape_key_object.data.shape_keys.key_blocks

            _retriever = [
                unique_shapekey_set.add((shapekey.name, shapekey.name, ""))
                for shapekey in key_blocks
                if (shapekey.name not in ["", "Basis"])
            ]
    ...
    shapekey_source_name: bpy.props.EnumProperty(
        name="source shapekeys", items=auto_retrieve_shapekeys
    )  # type: ignore
    ...
    def execute(self, context: bpy.types.Context):
        ...
        if self.shapekey_source_name not in ["", "NONE"]:
            ...
            if (properties.shapekey_transfer_source_object is not None) and (
                    properties.shapekey_transfer_source_object.type == "MESH"
            ):
                source_object: bpy.types.Object = (
                    properties.shapekey_transfer_source_object
                )
            ...
            if (properties.shapekey_transfer_target_object is not None) and (
                    properties.shapekey_transfer_target_object.type == "MESH"
            ):
                target_object: bpy.types.Object = (
                    properties.shapekey_transfer_target_object
                )
            ...
            if (self.shapekey_source_kdtree is not None) and (
                    self.shapekey_target_kdtree is not None
            ):

                shapekey_layer = self.source_bmesh.verts.layers.shape.get(
                    self.shapekey_source_name
                )
                ...
                for source_vert in self.source_bmesh.verts:
                    ...
                    co, target_index, distance = self.shapekey_target_kdtree.find(
                        original_co
                    )
                ...
                if (
                        self.target_bmesh.verts.layers.shape.get(self.shapekey_source_name)
                        is None
                ):
                    properties.shapekey_transfer_target_object.shape_key_add(
                        name=self.shapekey_source_name
                    )
                ...
                self.target_shapekey_layer = self.target_bmesh.verts.layers.shape.get(
                    self.shapekey_source_name
                )

                if self.target_shapekey_layer is not None:
                    for target_index_key in shape_key_co_set:
                        self.target_bmesh.verts[target_index_key][
                            self.target_shapekey_layer
                        ] = shape_key_co_set[target_index_key]
                else:
...
class Alx_OT_Shapekey_AddEmptyShapeKeys(bpy.types.Operator):
    ...
    blendshape_target_type: bpy.props.EnumProperty(
        name="blendshapes type:",
        default="VRM0",
        items=[("VRM0", "VRM 0", "", 1), ("ARKIT", "ARKit", "", 1 << 1)],
    )  # type: ignore
...
class ALX_OT_ShapeKey_MergeShapeKeys(bpy.types.Operator):
    """"""

    bl_label = "ALX Shapekeys - Merge Shapekeys"
    bl_idname = "alx.operator_shapekey_merge_shapekeys"
    bl_options = {"REGISTER", "UNDO"}

    merge_target: bpy.types.ShapeKey = None
    target_shapekeys: set[bpy.types.ShapeKey] = None
    ...
    def execute(self, context: bpy.types.Context):
        context_object: bpy.types.Object | None = context.object

        if context_object is not None:
            mesh: bpy.types.Mesh = context_object.data

            merged_shapekey_co = zip(
                *[key_block.points for key_block in mesh.shape_keys.key_blocks]
            )

            for i, co_set in enumerate(merged_shapekey_co):
                print(self.merge_target.points[i].co)  # = sum(co_set)
