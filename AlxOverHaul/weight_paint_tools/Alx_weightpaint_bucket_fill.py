...
class ALX_PT_Panel_WeightPaint_BucketFill(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        properties: Alx_PG_PropertyGroup_WeightPaint_BucketFill_Properties = (
            context.scene.alx_tool_weight_paint_bucket_fill_properties
        )
...
class ALX_OT_WeightPaint_BucketFill(bpy.types.Operator):
    ...
    def modal(self, context: bpy.types.Context, event: bpy.types.Event):
        ...
        if (
                ...
                != "alx.workspacetool_weightpaint_bucket_fill"
        ):
            return {"CANCELLED"}
            ...
            if (context.area is not None) and (context.area.type == "VIEW_3D"):
                if context.weight_paint_object is not None:
                    ...
                    if wp_object.vertex_groups.active is not None:
                        wp_object.vertex_groups.active.add(
                            [vert.index for vert in wp_object.data.vertices],
                            weight=context.scene.alx_tool_weight_paint_bucket_fill_properties.weight,
                            type="REPLACE",
                        )

                    else:
                        self.report(
                            {"INFO"}, "[object][missing] | [active][vertex group]"
                        )
            return {"PASS_THROUGH"}
        ...
        if (
            ...
            bpy.ops.wm.call_panel(
                name=ALX_PT_Panel_WeightPaint_BucketFill.bl_idname, keep_open=True
            )
            return {"RUNNING_MODAL"}
...
class ALX_WST_WeightPaint_BucketFill(bpy.types.WorkSpaceTool):
    ...
    bl_keymap = (
        (
            "alx.operator_weight_paint_bucket_fill",
