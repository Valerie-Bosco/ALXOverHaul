...
from .ALX_Alexandria_Layouts import (
    Alx_UL_UIList_ObjectSelectionModifiers,
...
from ..definitions.AlxTypesDefinition import (
    TD_modifier_deform_types,
...
from ..reorganize_later.AlxVisibilityOperators import Alx_OT_Scene_VisibilityIsolator
...
class Alx_UL_UIList_ObjectSelectionProperties(bpy.types.UIList):
    ...
    def draw_item(
        ...
        header.prop(
            item.ObjectPointer, "name", text="", icon="OBJECT_DATA", emboss=True
        )

        header.prop(
            item.ObjectPointer, "show_name", text="", icon="SORTALPHA", emboss=True
        )
        header.prop(
            item.ObjectPointer, "show_axis", text="", icon="EMPTY_ARROWS", emboss=True
        )
...
class Alx_PT_Panel_AlexandriaGeneralPanel(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        addon_properties: Alx_PG_PropertyGroup_SessionProperties = (
            context.window_manager.alx_session_properties
        )
        ...
        if (addon_properties.alexandria_general_panel_tabs == "MODIFIER") or (
                addon_properties.alexandria_general_panel_modifier_sidetabs != "CLOSED"
        ):
        ...
        header_grid = header_layout.grid_flow(
        ...
        with context.temp_override(
                window=override_window, area=override_area[0], region=override_region[0]
        ):
            row = header_slot_one.row().split(factor=0.5)
            UIPreset_PosePosition(
                row, context_skeleton, Alx_OT_Armature_Pose_SetPosePosition.bl_idname
            )
            header_slot_one.separator()

            row = header_slot_one.row().split(factor=0.5)
            UIPreset_VisibilityIsolator(
                row, addon_properties, Alx_OT_Scene_VisibilityIsolator.bl_idname
            )
            header_slot_one.separator()
        ...
        tabs = main_layout.column().prop(
            addon_properties,
            "alexandria_general_panel_tabs",
            icon_only=True,
            expand=True,
            emboss=False,
        )
        ...
        if addon_properties.alexandria_general_panel_tabs == "OBJECT":
            ...
            ObjectTabBox.template_list(
                Alx_UL_UIList_ObjectSelectionProperties.bl_idname,
                list_id="",
                dataptr=context.scene,
                propname="alx_object_selection_properties",
                active_dataptr=context.scene,
                active_propname="alx_object_selection_properties_index",
                type="DEFAULT",
                columns=2,
            )
        ...
        if addon_properties.alexandria_general_panel_tabs == "ARMATURE":
            ...
            if context_skeleton is not None:
                ...
                armature_display_options.row().prop(
                    bpy.data.armatures.get(context_skeleton.data.name),
                    "display_type",
                    expand=True,
                )
                armature_display_options.prop(
                    bpy.data.armatures.get(context_skeleton.data.name),
                    "show_names",
                    text="names",
                )
        ...
        if addon_properties.alexandria_general_panel_tabs == "MODIFIER":
            ...
            row = column.row()
            row.operator(
                Alx_OT_Modifier_BatchVisibility.bl_idname, text="Batch Visibility"
            )
            ...
            UIPreset_EnumButtons(
                layout=modifier_sidetabs,
                primary_icon="MODIFIER",
                data=addon_properties,
                data_name="alexandria_general_panel_modifier_sidetabs",
            )
            ...
            match addon_properties.alexandria_general_panel_modifier_sidetabs:
                ...
                case "PHYSICS":
                    UIPreset_ModifierCreateList(
                        layout=side_page_layout.row(),
                        modifiers_types=[["Physics", TD_modifier_physics_types]],
                        modifier_creation_operator=Alx_OT_Modifier_ManageOnSelected,
                    )
        ...
        if (addon_properties.alexandria_general_panel_tabs == "RENDER") and (
                b_override_v3d_area_exists
        ):
            ...
            if override_region is not None:
                with context.temp_override(
                    ...
                    if context.area.spaces.active.shading.type == "SOLID":
                        ...
                        matcap_preview.row().prop(
                            context.area.spaces.active.shading, "light", expand=True
                        )
                ...
                if context.area.spaces.active.shading.type == "MATERIAL":
                    ...
                    if context.area.spaces.active.shading.use_scene_world == True:
                        scene_shading_options.row().prop(
                            context.scene.world,
                            "use_nodes",
                            text="Use Scene World Nodes",
                            toggle=True,
                        )
                        ...
                        if context.scene.world.use_nodes == True:
                            if context.scene.world.node_tree is not None:
                                ...
                                MaterialOutput = (
                                    context.scene.world.node_tree.get_output_node("ALL")
                                )
                ...
                if context.area.spaces.active.shading.type == "RENDERED":
                    ...
                    if (
                            context.area.spaces.active.shading.use_scene_world_render
                            == False
                    ):
                        material_preview = scene_shading_options.row().split(factor=0.5)
                        material_preview.template_icon_view(
                ...
                if context.area.spaces.active.shading.type == "SOLID":
                    ...
                    solid_shading_options.row().prop(
                        context.area.spaces.active.shading, "show_cavity"
                    )
            ...
            if context.scene.render.engine == "CYCLES":
                ...
                cycles_samples.prop(
                    context.scene.cycles, "preview_samples", text="Viewport"
                )
...
class Alx_PT_Scene_GeneralPivot(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (bpy.app.version[0] == 4) and (bpy.app.version[1] in [0, 1, 2]):
            snapping_column.prop(
                context.tool_settings, "snap_elements_base", expand=True
            )
            snapping_column.prop(
                context.tool_settings, "snap_elements_individual", expand=True
            )
        ...
        snap_affect_type.prop(
            context.tool_settings, "use_snap_translate", text="Move", toggle=True
        )
        snap_affect_type.prop(
            context.tool_settings, "use_snap_rotate", text="Rotate", toggle=True
        )
        snap_affect_type.prop(
            context.tool_settings, "use_snap_scale", text="Scale", toggle=True
        )
