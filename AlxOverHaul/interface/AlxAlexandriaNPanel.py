...
from ..MeshTools import ALX_shapekey, AlxVertexGroupTools
...
class ALX_PT_UI_Addon_ToolShelf(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        mesh_tools_header, mesh_tools_panel = mainlayout.panel(
            "ALX_PT_compact_panel_mesh_tools", default_closed=True
        )
        ...
        if mesh_tools_panel is not None:
            ...
            layout.operator(
                AlxVertexGroupTools.Alx_OT_Mesh_VertexGroup_Clean.bl_idname,
                text="Clean VGroups",
            )

            layout.label(text="ShapeKeys:")
            layout.operator(
                ALX_shapekey.Alx_OT_Shapekey_TransferShapekeysToTarget.bl_idname,
                text="Transfer Shapekeys",
            )

            layout.label(text="UV/UDIMs:")
            layout.operator(
                AlxUDIMTools.Alx_OT_UVTools_udim_square_compressor.bl_idname,
                text="Compress UDIMs",
            )

        armature_tools_panel: bpy.types.UILayout
        armature_tools_header, armature_tools_panel = mainlayout.panel(
            "ALX_PT_compact_panel_armature_tools", default_closed=True
        )
        ...
        if armature_tools_panel is not None:
            ...
            layout.operator(
                Alx_rigging_tools.ALX_OT_SKELETON_AutoResolveEndBones.bl_idname,
                text='Resolve "_end" Bones',
            )
            layout.operator(
                Alx_rigging_tools.ALX_OT_SKELETON_RemoveEndBones.bl_idname,
                text='Remove "_end" Bones',
            )

            layout.label(text="Pose:")
            layout.operator(
                Alx_pose_tools.Alx_OT_Armature_Pose_ToggleConstraints.bl_idname,
                text="Toggle Pose Constraints",
            )
            layout.operator(
                Alx_pose_tools.Alx_OT_Armature_MatchIKByMirroredName.bl_idname,
                text="Symmetrize IK",
            )

        vtuber_panel: bpy.types.UILayout
        vtuber_header, vtuber_panel = mainlayout.panel(
            "ALX_PT_compact_panel_vtuber_tools", default_closed=True
        )
        ...
        if vtuber_panel is not None:
            ...
            layout.label(text="ShapeKeys:")
            layout.operator(
                ALX_shapekey.Alx_OT_Shapekey_AddEmptyShapeKeys.bl_idname,
                text="Add Empty Shapekeys",
            )

            layout.label(text="Rigging:")
            layout.operator(
                Alx_rigging_tools.Alx_OT_Armature_BoneChainOnSelection.bl_idname,
                text="Autorig Strip",
            )
