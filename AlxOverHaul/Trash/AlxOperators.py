            ...
            if ContextBMesh is not None:
                    ...
                    if SelectedEdge is not None:
                        ...
                        if SelectedEdge.is_boundary == True:
                            BoundaryVertex.extend(
                                [
                                    Vertex
                                    for Vertex in SelectedEdge.verts
                                    if (Vertex not in BoundaryVertex)
                                ]
                            )
                ...
                if self.UsePin == True:
                    ...
                    if self.KeepDividingEdges == False:
                        RemoveVertex = [
                            Vertex.index
                            for Edge in DividingNonBoundaryEdge
...
class Alx_OT_Mesh_EditAttributes(bpy.types.Operator):
    ...
    AttributeDomainType: bpy.props.EnumProperty(
        name="Attribute Type",
        default=0,
        items=[("POINT", "Vertex", ""), ("EDGE", "Edge", "")],
    )
    AttributeType: bpy.props.EnumProperty(
        name="Attribute Type", default=0, items=[("FLOAT_COLOR", "Float Color", "")]
    )
        ...
        try:
            ...
            if context.mode == "EDIT_MESH":
                ...
                for MeshObject in MeshSelection:
                    ...
                    SelectedVertex = [
                        Vertex.index
                        for Vertex in ContextBMesh.verts
                    ...
                    if ContextAttribute is None:
                        if self.CreateMissingAttribute == True:
                            ContextAttribute = MeshObject.attributes.new(
                                ...
                                domain=self.AttributeDomainType,
                            )
                        ...
                        if self.AttributeDomainType == "POINT":
                            for Vertex in SelectedVertex:
                                if self.AttributeType == "FLOAT_COLOR":
                                    ContextAttribute.data[Vertex].color = (
                                        self.ColorValue
                                    )
...
class Alx_OT_Armature_AssignToSelection(bpy.types.Operator):
    ...
    UsePurgeOtherGroups: bpy.props.BoolProperty(
        name="PURGE Other VxGroups",
        description="PURGE any other VxGroup that does not correspon to the specified single VxGroup",
        default=False,
        update=UpdateUsePurgeOtherGroups,
    )
                ...
                if armature is not None:
                    if SelectedObject.type == "MESH":
                        ...
                        if self.UseParent == True:
                            ...
                            SelectedObject.matrix_parent_inverse = (
                        ...
                        if self.UseSingleVxGroup == True:
                            if self.CreateMissingVxGroups == True:
                                if (self.VxGroupName != "") and (
                                        self.VxGroupName not in SelectedObject.vertex_groups
                                ):
                                    VxGroup = SelectedObject.vertex_groups.new()
                                    VxGroup.name = self.VxGroupName
                            if (self.VxGroupName != "") and (
                                    self.VxGroupName in SelectedObject.vertex_groups
                            ):
                                ...
                                if VxGroup is not None:
                                    VxGroup.add(
                                        range(0, len(SelectedObject.data.vertices)),
                                        1.0,
                                        "REPLACE",
                                    )
    ...
    def execute(self, context):
        ...
        for Object in bpy.context.selected_objects:
            if (
                    ...
                    and (Object.type in ["MESH", "ARMATURE", "CURVE"])
            ):
                ObjectModfiers = getattr(Object, "modifiers", [])

                for ObjectModifier in ObjectModfiers:
                    if (ObjectModifier is not None) and (
                            ObjectModifier.type in ModifiersList
                    ):
