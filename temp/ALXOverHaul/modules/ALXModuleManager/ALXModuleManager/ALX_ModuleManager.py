...
class Alx_Module_Manager:
    ...
    def developer_register_modules(self):
        self.__module_folders = self.__gather_addon_folders(
            self.__module_path, self.__folder_blacklist
        )
        self.__module_files = self.__gather_addon_files(
            self.__module_folders, self.__file_blacklist
        )
    ...
    def developer_load_resources(
            self, icons_definitions: list[dict["name":str, "path":str, "resource_type":str]]
    ):
        """
        ...
        for entry in icons_definitions:
            ...
            if {"name", "path", "resource_type"}.issubset(set(entry.keys())):
                path_object = Path(
                    f"{self.__module_path}{os_separator if self.__module_path[-1] != os_separator else ''}{entry['path']}"
                )
    ...
    def __gather_addon_files(
            self, folder_paths: set[Path], file_blacklist: set[str] = {}
    ):
        """
        ...
        for file_name in addon_files.keys():
            if (file_name != __file__) and (file_name not in self.__file_blacklist):
                for addon_class in getmembers(
                        eval(file_name, self.__init_globals), isclass
                ):
    ...
    def __execute_locals_update(self, path: str, addon_files: dict[str, Path]):
        for file_name in addon_files.keys():
            if (file_name != __name__.split(".")[-1]) and (
                    file_name not in self.__file_blacklist
            ):
                try:
                    ...
                    if file_name not in self.__init_globals:
                        relative_path = str(
                            addon_files.get(file_name).relative_to(path)
